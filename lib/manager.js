...
const manager = {
  ...
  initConfig(saveOnChange = false) {
    ...
    // Transpiler.
    switch (this.config.transpiler) {
      case 'babel':
        this.transpiler = babel;
        break;
      default:
        // This block should never be reached, since no unsupported transpilers allowed by
        // `joi` checking & enforcing and each supported transpiler should have its own case.
        throw new Error(`Unsupported transpiler "${this.config.transpiler}"`);
    }
    this.transpiler.ensureConfig(this.config, saveOnChange);

    // Coverage tool.
    switch (this.config.instrumenter) {
      case 'nyc':
        this.instrumenter = nyc;
        break;
      default:
        ...
        throw new Error(`Unsupported instrumenter "${this.config.transpiler}"`);
    }
    this.instrumenter.ensureConfig(this.config, saveOnChange);

    // Internal path where to save the instrumented files.
    ...
    // TODO Save new config to `.atom-coverage` (to avoid recalc `sourcesRoot`,...).
  },
