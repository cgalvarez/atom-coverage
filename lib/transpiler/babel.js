...
const { findConfigFile, readConfig, writeConfig } = require('../util');
...
const babel = {
  ...
  readConfig() {
    this.config.filepath = findConfigFile('.babelrc');
    this.config.settings = this.config.filepath
      ? readConfig(this.config.filepath, 'json')
      : {};
  },

  saveConfig() {
    writeConfig(this.config.filepath || '.babelrc', this.config.settings, 'json');
  },
